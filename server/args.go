...
func ParseExporterArgs() ExporterArgs {
	args := ExporterArgs{
		...
		).Short('w').OverrideDefaultFromEnvar("HADOOP_METRIC_EXPORTER_DISCOVERY_WHITELIST").String(),
		Address: kingpin.Flag(
			"address",
			"Polling server on this address (hostname or ip). Default: 0.0.0.0",
		).Default("0.0.0.0").Short('h').OverrideDefaultFromEnvar("HADOOP_METRIC_EXPORTER_ADDRESS").String(),
		Port: kingpin.Flag(
			"port",
			"Port to listen on. Default: 9123",
		).Default("9123").Short('p').OverrideDefaultFromEnvar("HADOOP_METRIC_EXPORTER_PORT").Int(),
		MetricPath: kingpin.Flag(
			"metricPath",
			"Path under which to expose metrics. Default: /metrics",
		).Default("/metrics").Short('d').OverrideDefaultFromEnvar("HADOOP_METRIC_EXPORTER_METRIC_PATH").String(),
		LogLevel: kingpin.Flag(
			"logLevel",
			"Log level, include: all, debug, info, warn, error. Default: info",
		).Default("info").Short('l').OverrideDefaultFromEnvar("HADOOP_METRIC_EXPORTER_LOG_LEVEL").String(),
	}
	kingpin.Parse()
	return args
}
